...
public class TextReaderBonus extends JPanel implements ActionListener {
    ...
    public static JComboBox comboBoxChapter;
    public static String[] options = {"moby.txt"};

    // Constructor of JPanel Object
    public TextReaderBonus() throws IOException {

        // Add JButton and set the bounds of where it is located
        add(addFile = new JButton("Add File"));
        addFile.setBounds(new Rectangle(50, 80, 400, 50));
        ...
        comboBox.setBounds(new Rectangle(50, 140, 400, 25));
        ...
        comboBoxChapter.setBounds(new Rectangle(50, 170, 400, 25));

        comboBoxChapter.setActionCommand("combo_box_change_state_chapter");
        comboBoxChapter.addActionListener(this);
    ...
    // Desc: appropriately changes the chapter list on the combo box
    // Param: N/A
    // Return: N/A
    public static void updateChapterList() throws IOException {

        String dir = Objects.requireNonNull(comboBox.getSelectedItem()).toString();

        // Keep track of how many chapters we have iterated over so far
        int chapterCount = 0;
        ...
        while (input != null) {
            if (input.length() >= 12) {
                if (input.substring(0, 12).equalsIgnoreCase(".. < chapter")) chapterCount++;
            }
            input = in.readLine();
        }

        comboBoxChapter.removeAllItems();
        comboBoxChapter.addItem("All Chapters");

        for (int i = 1; i < chapterCount + 1; i++) {
            comboBoxChapter.addItem("Chapter " + i);
        }
    }
    ...
    // Desc: takes in a directory and adds all the unique words to a treemap, making sure to keep track of how many times every word occurs in the chapter specified
    // Param: takes in the directory of the file to parse in string format and the chapter to look through
    // Return: N/A
    public static void findWords(String dir, int chapter) throws IOException {

        // Start the timer and initialize StringBuilder
        long startTime = System.nanoTime();
        StringBuilder file = parseInput(dir, chapter);
    ...
    // Desc: takes in the directory of a file and chapter and combines it into one StringBuilder
    // Param: takes in the directory of the file to parse in string format and the chapter to parse
    // Return: a StringBuilder of the chapter
    public static StringBuilder parseInput(String dir, int chapter) throws IOException {
        ...
        input = in.readLine();
        if (input.length() >= 12) {
            if (input.substring(0, 12).equalsIgnoreCase(".. < chapter")) input = null;
        }
        ...
        while (input != null) {
            ...
            if (input != null) {
                if (input.length() >= 12) {
                    if (input.substring(0, 12).equalsIgnoreCase(".. < chapter")) break;
                }
            }
        }
    ...
    // Param: takes in the directory of the file to parse in string format
    // Return: a StringBuilder of the file
    ...
    public String[] appendOptionToArray(String append) {
        ...
        // Loop through all elements and convert them to the new array
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (event.equals("add_file")) {
            ...
            comboBoxChapter.setSelectedItem("All Chapters");
        ...
        } else if (event.equals("combo_box_change_state_chapter")) {

            if (comboBoxChapter.getSelectedItem() != null) {
                if (comboBoxChapter.getSelectedItem().toString().equalsIgnoreCase("All Chapters")) {
                    try {
                        textArea.setText(new String(Files.readAllBytes(Paths.get(comboBox.getSelectedItem().toString()))));
                    } catch (IOException ex) {
                        ex.printStackTrace();
                    ...
                    int chapter = Integer.parseInt(comboBoxChapter.getSelectedItem().toString().substring(8));

                    try {
                        textArea.setText(parseInput(comboBox.getSelectedItem().toString(), chapter).toString());
                    } catch (IOException ex) {
                        ex.printStackTrace();
                    }

                    try {
                        findWords(comboBox.getSelectedItem().toString(), chapter);
                    } catch (IOException ex) {
                        ex.printStackTrace();
                    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        // Initialize JPanel
        JPanel TextReaderBonus = new TextReaderBonus();
